...
public class Bunker implements GameObject, Renderable, Prototype {
    ...
    public Prototype copy() {
        ...
        builder.reset();
        builder.buildPosition(new Vector2D(position.getX(), position.getY()));
        builder.buildLives(lives);
        builder.buildImage(new Image(image.getUrl(), 25, 25, true, true));

        Bunker bunkerCopy = (Bunker) builder.createGameObject();

        if (state instanceof GreenState) {
            builder.setState(new GreenState(bunkerCopy));
        }
        else if (state instanceof YellowState) {
            builder.setState(new YellowState(bunkerCopy));
        }
        else if (state instanceof RedState) {
            builder.setState(new RedState(bunkerCopy));
        }
